...
class UserListView(APIView):
    def get(self, request):
        ...
        return Response([{'id': user.id, 'username': user.username, 'unread_count': Message.objects.filter(recipient=request.user, sender=user, is_read=False).count()} for user in users])
